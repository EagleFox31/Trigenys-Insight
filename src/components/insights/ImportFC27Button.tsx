...
export function ImportFC27Button() {
  ...
  async function runImport() {
    ...
    setEditUrl('')
    try {
      const response = await fetch('/api/editorial/import-fc27', {
        method: 'POST',
        credentials: 'include',
      })
  ...
  return (
    <div className="mt-6">
      <button className="min-h-12 rounded-md bg-[#102f52] px-6 py-3 font-bold text-white disabled:opacity-60" disabled={busy} onClick={runImport} type="button">
        {busy ? 'Import en cours…' : 'Importer FC 27 dans Payload'}
      </button>
