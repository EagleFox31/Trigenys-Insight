...
export function ArticleShareButtons({ locale, slug, title, url, analyticsEnabled = true }: Props) {
  ...
  return (
    <section className="mt-5 border-t border-border pt-4" aria-label={locale === 'fr' ? 'Partager cet article' : 'Share this article'}>
      <p className="mb-3 text-sm font-semibold text-foreground">{locale === 'fr' ? 'Partager' : 'Share'}</p>
      <div className="flex flex-wrap items-center gap-2">
        {links.map(({ name, href }) => (
          <a key={name} href={href} target="_blank" rel="noopener noreferrer" onClick={() => track(name.toLowerCase())}
            ...
            className="inline-flex min-h-10 items-center rounded-full border border-border px-4 text-sm text-foreground transition-colors hover:bg-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary">
            {name}
          </a>
        ))}
        <button type="button" onClick={copy} className="inline-flex min-h-10 items-center gap-2 rounded-full border border-border px-4 text-sm text-foreground transition-colors hover:bg-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary">
